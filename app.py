    ...
    def _save(self):
        ...
        try:
            ...
            while n < 1000:
                ...
                messagebox.showerror(
                    "Save failed",
                    "Could not find a free cite key for this item.",
                    parent=self)
                return
...
class CollectorCatalogApp:
    ...
    def _new_item(self):
        ...
        # Step 2: the ISBN tile opens the rapid add-by-barcode loop.  The
        # plain Book tile goes straight to the blank form, as asked — no
        # lookup dialog in the way.
        if dlg.mode == "isbn":
            self._add_books_by_isbn(type_id, coll_id)
            return

        # Step 3: create the blank item and open it for editing.
